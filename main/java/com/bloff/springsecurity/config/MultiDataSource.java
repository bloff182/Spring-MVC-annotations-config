...
/*
 * We use that class when we use the data from two other databases. 
  We use db do login & we load info users from db
 * To load customer's data we use another db
 * To use it in securityconfiguration.class injected securityDataSource  
 */
...
@ComponentScan(basePackages="com.bloff.springsecurity")
@PropertySource({"classpath:customer-persistence-mysql.properties","classpath:security-persistence-mysql.properties"})
public class MultiDataSource implements WebMvcConfigurer  {
	...
	public DataSource securityDataSource() {
		...
		DataSource theDataSource = buildDataSource("security");
	...
	public LocalSessionFactoryBean securitySessionFactory() {
		...
		sessionFactory.setHibernateProperties(getHibernateProperties("security"));
	...
	@Bean
	@Autowired
	public HibernateTransactionManager employeeTransactionManager(@Qualifier("securitySessionFactory") SessionFactory sessionFactory) {
